...
BMO_egress_Blood <- readRDS(
  file.path(project_root, "data", "processed", "BMO_egress_Blood.rds")
)
...
Blood_Erythroid[["RNA"]] <- split(Blood_Erythroid[["RNA"]], f = Blood_Erythroid$orig.ident)

Blood_Erythroid <- NormalizeData(Blood_Erythroid)
Blood_Erythroid <- FindVariableFeatures(Blood_Erythroid, selection.method = "vst", nfeatures = 2000)

all.genes <- rownames(Blood_Erythroid)
Blood_Erythroid <- ScaleData(Blood_Erythroid, features = all.genes)

Blood_Erythroid <- RunPCA(Blood_Erythroid, features = VariableFeatures(object = Blood_Erythroid))

Blood_Erythroid <- IntegrateLayers(
  object = Blood_Erythroid,
...
Blood_Erythroid[["RNA"]] <- JoinLayers(Blood_Erythroid[["RNA"]])

DimHeatmap(Blood_Erythroid, dims = 1:30, cells = 500, balanced = TRUE)
ElbowPlot(Blood_Erythroid, ndims = 30)

Blood_Erythroid <- FindNeighbors(Blood_Erythroid, reduction = "integrated.cca", dims = 1:10)
Blood_Erythroid <- FindClusters(Blood_Erythroid, resolution = 1)

Blood_Erythroid <- RunUMAP(Blood_Erythroid, reduction = "integrated.cca", dims = 1:10)
Blood_Erythroid <- RunTSNE(Blood_Erythroid, reduction = "integrated.cca", dims = 1:10)

p1 <- DimPlot(Blood_Erythroid, reduction = "tsne", label = TRUE) + ggtitle("t-SNE")
p2 <- DimPlot(Blood_Erythroid, reduction = "umap", label = TRUE) + ggtitle("UMAP")

DimPlot(Blood_Erythroid, reduction = "umap", label = TRUE, group.by = "celltype") + ggtitle("UMAP")
DimPlot(Blood_Erythroid, reduction = "tsne", label = TRUE, group.by = "celltype") + ggtitle("TSNE")

p1+p2

# Erythroid
FeaturePlot(Blood_Erythroid, features = c("TFRC","GYPA","SLC4A1","KLF1"), max.cutoff = 3, min.cutoff=0,
